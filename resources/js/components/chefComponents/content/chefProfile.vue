<template>
    <main class="profile-main" v-if="!recipeDetailsVisible && ProfileVisible">
      ...
      <div class="user-info-con">
        <h2>{{ chef.fullname }} <a href="#" id="edit-profile-button" @click.prevent="toggleEditProfile()"><img src="/public/images/edit.png" alt="edit"></a></h2>
        <p>{{ chef.account.email }}</p>
        <p>@{{ chef.username }}</p>
...
// Handle toggling the reaction (like/dislike) for a recipe
...
const form = ref({
  fullname: props.chef.fullname,
  username: props.chef.username,
});
  
function updateProfile() {
  Inertia.post(route('chef.profile.update'), form.value, {
    preserveScroll: true,
    onSuccess: () => {
      editProfileVisible.value = false;
...
.profile-photo{
    ...
    top: 15%;
    left: 5%;
    background-image: url('../images/add-profile.png');
